...
def spam_handler():
    ...
    phone = parse_phone(input(f"{BRIGHT}{BLUE}Torpeda >> {RESET_ALL}"))
    print()
    print("Использовать прокси? (y/n)")
    proxy = input(f"{BRIGHT}{BLUE}Torpeda >> {RESET_ALL}")
    if proxy.lower() == "y":
    ...
    threads = input(f"{BRIGHT}{BLUE}Torpeda >> {RESET_ALL}")
    if threads in ["", " ", "0"]:
        threads = "1"
    try:
        if int(threads) > 10:
...
def update():
    ...
    update = input(f"{BRIGHT}{BLUE}Torpeda >> {RESET_ALL}")
    if update.lower() == "y":
        if exists("/usr/bin") and isfile("/usr/bin/Torpeda"):
            file = open("/usr/bin/Torpeda", "wb")
        elif exists("/usr/local/bin/") and isfile("/usr/local/bin/Torpeda"):
            file = open("/usr/local/bin/Torpeda", "wb")
        ...
        try:
            ...
            system("cd $HOME && rm -rf Torpeda && git clone https://github.com/nordbearbotdev/Torpeda && cd Torpeda && sh install.sh")
    else:
        main()
